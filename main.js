...
const fraction = (num) => {
  ...
  if (num == parseInt(num)) {
    ...
    while (
      parseFloat(num) * Math.pow(10, flag) !==
      parseInt(parseFloat(num) * Math.pow(10, flag))
    ) {
      flag += 1;
    }
...
const det = () => {
  try {
    ...
    if (detCalc(detMatrix)[1] === 1) {
      ...
      if (a % b === 0) {
        document.getElementById("detValue").innerHTML =
          document.getElementById("detValue").innerHTML + a / b;
      } else {
    ...
    document.getElementById("detValue").innerHTML =
      document.getElementById("detValue").innerHTML + "Error";
  }
};
  ...
  if (len >= 3) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (i === 0) {
        ...
        section = arr
          .slice(1, len)
          .map((j) => [].concat(j.slice(0, i), j.slice(i + 1, len)));
      }
      sum = fracSum(
        sum,
        fracMul(
          [arr[0][i][0] * Math.pow(-1, i), arr[0][i][1]],
          detCalc(section)
        )
      );
    }
    ...
    return fracSum(
      fracMul(arr[0][0], arr[1][1]),
      fracMul([arr[0][1][0] * Math.pow(-1, 1), arr[0][1][1]], arr[1][0])
    );
  }
};
        ...
        for (let j = 0; j < val; j++) {
          if (j !== val - 1) {
            ...
            if (inverse[i][j][0] === 0) {
              ...
              jaxMatrix[i] +=
                `${inverse[i][j][0]}` + slash.replace("{", "").replace("}", "");
            } else {
              jaxMatrix[i] +=
                "\\frac{" +
                `${inverse[i][j][0]}` +
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let p = 0; p < len; p++) {
      if (p !== i && parseInt(arra[p][i][0]) !== 0) {
        conti = ideal[p].map((m, index) => {
          return fracSub(m, fracMul(arra[p][i], ideal[i][index]));
        });
        conta = arra[p].map((m, index) => {
          return fracSub(m, fracMul(arra[p][i], arra[i][index]));
        });
        for (let k = 0; k < len; k++) {
          ideal[p][k] = conti[k];
          arra[p][k] = conta[k];
